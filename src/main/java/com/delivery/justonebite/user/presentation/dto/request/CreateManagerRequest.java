...
@Schema(description = "관리자(MANAGER) 생성 요청 DTO")
@Builder
public record CreateManagerRequest(
        @Schema(description = "인증에 필요한 이메일(고윳값)", example = "manager1@email.com")
        @NotBlank(message = "이메일을 입력해주세요.")
        @Email
        String email,
        @Schema(description = "생성할 유저의 이름", example = "111")
        @NotBlank(message = "이름을 입력해주세요.")
        String name,
        @Schema(description = "생성할 유저의 전화번호", example = "010-1111-1111")
